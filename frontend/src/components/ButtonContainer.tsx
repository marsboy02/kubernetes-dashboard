...
const ButtonContainer = ({ deployment, namespace }) => {
  ...
  return (
    <Box sx={{ display: "flex", gap: 2 }}>
      <Box>
        <Button size="large" onClick={() => setReplicasPopoverOpen(true)}>
          set replicas
        </Button>
        <Popover open={replicasPopoverOpen}>
          ...
          >
            <Box as="form">
              <Heading sx={{ fontSize: 2 }}>레플리카 입력</Heading>
              <TextInput
                type="number"
                min={1}
                value={replicasNumber}
                onChange={(e) => setReplicasNumber(e.target.value)}
              />
              <Box sx={{ display: "flex", gap: 2 }}>
                <Button onClick={() => setReplicasPopoverOpen(false)}>
                  close
                </Button>
        ...
        <Popover open={imagePopoverOpen}>
          ...
          >
            <Box as="form">
              ...
              />
              <Box sx={{ display: "flex", gap: 2 }}>
                <Button onClick={() => setImagePopoverOpen(false)}>
                  close
                </Button>
